...
internal class NativePadInput(
    ...
    fun setActive(value: Boolean) {
        ...
        if (value) {
            val connections = registry.activate(discoverCandidates())
    ...
    fun onKey(event: KeyEvent): Boolean {
        ...
        if (keyCode == KeyEvent.KEYCODE_MENU ||
            ...
            if (event.action == KeyEvent.ACTION_DOWN && (connection == null || connection.supported)) {
        ...
        if (connection?.supported == false) return true
        val state = if (connection == null) keyboardState else padStates.get(event.deviceId) ?: return true
    ...
    fun onMotion(event: MotionEvent): Boolean {
        ...
        if (!connection.supported) return true
    ...
    private fun onDeviceRemoved(deviceId: Int) {
        ...
        if (active) {
            for (replacement in registry.snapshot()) {
                if (replacement.supported && padStates.get(replacement.deviceId) == null) {
                    addPadState(replacement)
                }
            }
        }
    ...
    private fun discoverCandidates(): List<NativeControllerCandidate> = buildList {
        for (deviceId in InputDevice.getDeviceIds()) candidateFor(deviceId)?.let(::add)
    }

    private fun candidateFor(deviceId: Int): NativeControllerCandidate? {
        val device = InputDevice.getDevice(deviceId) ?: return null
        if (!isPhysicalGamepad(device)) return null
    ...
    private fun isPhysicalGamepad(device: InputDevice): Boolean {
        ...
        val gamepad = device.supportsSource(InputDevice.SOURCE_GAMEPAD)
        val joystick = device.supportsSource(InputDevice.SOURCE_JOYSTICK)
        ...
        } || (device.isExternal && device.hasKeys(
