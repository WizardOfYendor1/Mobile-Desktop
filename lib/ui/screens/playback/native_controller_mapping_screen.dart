...
class NativeControllerMappingScreenState
  ...
  String _buttonLabel(RetroPadButton button) {
    final port = _device?.port;
    if (port == null) return button.label;
    final description = widget.inputDescriptors.describe(
      port,
      button.retroPadIndex,
    );
    if (description == null) return button.label;
    // "D-pad Up (D-Pad Up)" and "Start (Start)" are noise; a core that just
    // restates the button name has told the user nothing. Compared loosely so
    // punctuation and casing differences still count as a restatement.
    if (_squashed(description) == _squashed(button.label)) return button.label;
