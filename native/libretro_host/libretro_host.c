...
typedef void(RETRO_CALLCONV *fn_set_controller_port_device)(unsigned,
                                                            unsigned);
...
// SET_INPUT_DESCRIPTORS is a borrowed array terminated by an entry whose
// description is NULL, exactly like SET_CONTROLLER_INFO above: cores commonly
// free or replace it once this callback returns, so no pointer from it may
// escape this function. Build a complete new snapshot before publishing it,
// leaving a reader with either the old whole list or the new whole list
// (never a half-rebuilt list).
static bool copy_input_descriptors(struct lh_host *h,
  ...
  // Cores differ in when they send descriptors -- during retro_load_game, or
  // only after retro_set_controller_port_device. Logging the moment and the
  // shape of each advertisement is what distinguishes "the core never sent
  // any" from "the frontend read them too early".
  // diagnostic_log, not host_log: host_log feeds cb.message, which is
  // user-facing overlay state. A core's input inventory belongs in logcat only,
  // exactly as the controller inventory above it does.
