...
int lh_load(lh_host *h, const char *core_path, const char *rom_path,
  ...
  // it so a previous game's cores/ports don't leak a stale "reads analog"
  // signal into new content.
...
static int restart_core(struct lh_host *h) {
  ...
  // so a restart clears it even though it keeps most other state (e.g.
  // controller_devices, reapplied below) alive across the new core instance.
  atomic_store(&h->analog_queried_ports, 0u);
